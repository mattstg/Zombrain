...
{
	/// <summary>This object allows you to switch D2D material shaders between standard shader usage, and scriptable render pipelines (SRP) shaders.</summary>
	[ExecuteInEditMode]
	[HelpURL(D2dHelper.HelpUrlPrefix + "D2dRenderingPipeline")]
	public class D2dRenderingPipeline : ScriptableObject
	{
		public const string Title = "Destructible 2D";

		public const string Prefix = "D2D";
		...
		public static TextAsset Instance
		{
			get
			{
				if (instance == null)
				{
					instance = Resources.Load<TextAsset>(typeof(D2dRenderingPipeline).Name);
				}
...
namespace Destructible2D
{
	[CustomEditor(typeof(D2dRenderingPipeline))]
	public class D2dRenderingPipeline_Editor : Editor
	{
		public override void OnInspectorGUI()
		{
			var tgt = (D2dRenderingPipeline)target;

			EditorGUI.BeginDisabledGroup(true);
				EditorGUILayout.Toggle(new GUIContent("Is Scriptable"), D2dRenderingPipeline.IsScriptable);
			EditorGUI.EndDisabledGroup();

			EditorGUILayout.HelpBox("This tool allows you to quickly switch " + D2dRenderingPipeline.Title + " between using the Standard graphics pipeline, and the scriptable graphics pipeline.", MessageType.Info);

			EditorGUILayout.HelpBox("If you upgrade " + D2dRenderingPipeline.Title + ", then you may need to click the Switch To ___ Pipeline button again.", MessageType.Info);
